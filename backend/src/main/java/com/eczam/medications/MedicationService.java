...
public class MedicationService {
     ...
     * long catalog name/generic-name/active-ingredient rather than the whole
     * string — dropping anything below {@link #minSimilarity} so an
     * unrelated query returns nothing. {@code category} optionally narrows
     * to a single top-level therapeutic category (see {@link #categories()}).
     */
    @Transactional(readOnly = true)
    public SearchPage search(String q, String category, String cursor, int limit) {
        int effectiveLimit = clampLimit(limit);
        boolean blank = q == null || q.isBlank();
        String effectiveCategory = category == null || category.isBlank() ? null : category;
        ...
        if (blank) {
            ...
            rows = searchRepo.searchBySimilarity(
                    q.trim(), effectiveCategory, minSimilarity,
    ...
    static MedicationView toView(MedicationSearchRepository.Row r) {
        return new MedicationView(r.id().toString(), r.name(), r.genericName(),
                r.manufacturer(), r.barcode(), r.form(), r.strength(), r.categoryPath(), r.vectorIndexed());
    }
    static MedicationDetail toDetail(Medication m) {
        return new MedicationDetail(m.getId().toString(), m.getName(), m.getGenericName(),
                m.getManufacturer(), m.getBarcode(), m.getForm(), m.getStrength(), m.getCategoryPath(),
