...
import 'package:medtrack/ui/medication/medication_detail_screen.dart';
...
class _CabinetScreenState extends State<CabinetScreen>
  ...
  void _select(String? id, [BuildContext? itemContext]) {
  ...
  Future<void> _logDose(AppState app, Medication med) async {
    ...
    try {
      ...
      if (!mounted) return;
      _showSnack(e.code == 'INSUFFICIENT_STOCK'
          ? 'Not enough ${med.name} left — update the quantity first.'
          : e.message);
    }
  }

  void _openInfo(Medication med) {
    final catalogId = med.catalogId;
    if (catalogId == null || catalogId.isEmpty) {
      _showSnack('Connect to view this medicine\'s leaflet.');
      return;
    }
    _select(null);
    Navigator.of(context).push(MaterialPageRoute<void>(
      builder: (_) => MedicationDetailScreen(catalogId: catalogId, name: med.name),
    ));
  }
