                    ...
                    Semantics(
                      ...
                      child: IconButton(
                        ...
                        icon: const Icon(Icons.menu_book_outlined,
                            size: 20, color: MedColors.tealTextSoft),
                        onPressed: onInfo,
                      ),
                    ),
                  ],
...
/// Quantity + low-stock + expiry status badges, sourced from the backend
/// inventory fields on [Medication].
class _InventoryChips extends StatelessWidget {
  final Medication med;
  const _InventoryChips({required this.med});
  ...
  Widget build(BuildContext context) {
    ...
    return Wrap(
      ...
      runSpacing: 6,
      children: [
        _chip('$qtyLabel ${med.unit}', const Color(0x1A6EEBE4),
            const Color(0xD9BEEBE8)),
        if (med.lowStock)
          _chip('Low stock', const Color(0x33B98A2E), const Color(0xFFF2D38A)),
        switch (med.expiryStatus) {
          ExpiryStatus.expired =>
            _chip('Expired', const Color(0x33D8483A), MedColors.dangerSoft),
          ExpiryStatus.expiringSoon =>
            _chip('Expiring soon', const Color(0x33B98A2E),
                const Color(0xFFF2D38A)),
          ExpiryStatus.ok => const SizedBox.shrink(),
        },
      ],
    );
  }
