...
/// `medId` is the backend `user_medications` UUID (contains no `|`).
class ReminderPayload {
  ...
  static ReminderPayload? tryDecode(String? raw) {
    ...
    if (medId.isEmpty || minute == null || index == null) return null;
...
/// so it talks to the mirror database directly. Taken doses are written with
/// sync_state='local'; the sync engine pushes them to the backend on resume.
  ...
  switch (response.actionId) {
    case kActionTaken || kActionSkipped:
      ...
      await SqliteMedicationRepository(db).upsertLog(
        ...
        // taken → 'local' so the sync engine pushes it to the backend on the
        // next foreground/reconnect; skipped stays local forever.
        syncState: 'local',
      );
    case kActionSnooze:
...
class NotificationService {
  ...
  /// Folds a backend UUID into a bounded positive 32-bit-safe key. The local
  /// notification plugin keys reminders by int id; med N owns
  /// `[key*100, key*100+62)` so daily (0..kMaxReminders-1) and snooze (+50)
  /// ids never collide for one med. Cross-med collisions are astronomically
  /// unlikely for a personal cabinet.
  static int keyForId(String medId) => (medId.hashCode & 0x7fffffff) % 20000000;

  static int dailyNotificationId(String medId, int index) =>
      keyForId(medId) * 100 + index;

  static int snoozeNotificationId(String medId, int index) =>
      keyForId(medId) * 100 + 50 + index;
  ...
  Future<void> scheduleForMedication(Medication med) async {
    if (med.id.isEmpty) return; // not yet persisted; nothing stable to key on
