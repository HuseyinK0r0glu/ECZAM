...
/// fake-async zone. Mirrors the behaviour the UI relies on: insert assigns a
/// (String) id, log upsert keyed by (medId, minute, dateKey), and ordering.
class FakeMedicationRepository implements MedicationRepository {
  ...
  Future<String> insertMedication(Medication med) async {
    final id = med.id.isEmpty ? 'fake-${++_medSeq}' : med.id;
    _meds.add(med.copyWith(id: id));
    return id;
  }
...
class FakeNotificationService extends NotificationService {
  final List<String> scheduledMedIds = [];
  final List<String> cancelledMedIds = [];

  /// Exact reminder minutes per medication id from the last schedule call.
  final Map<String, List<int>> scheduledMinutes = {};
